...
st.set_page_config(
    page_title="Nutrisense - Automatic Diet Recommendation",
    page_icon="D",
    layout="wide",
...
class Person:
    ...
    def generate_recommendations(self,):
        ...
        for meal in self.meals_calories_perc:
            ...
            # Adjust protein content based on weight gain/loss plan
            if self.weight_loss > 1.0:  # Weight gain plans
                # Increase protein for weight gain
                recommended_nutrition[8] = rnd(80,200)  # Higher protein for weight gain
            elif self.weight_loss < 1.0:  # Weight loss plans
                # Moderate protein for weight loss
                recommended_nutrition[8] = rnd(50,150)
...
class Display:
    def __init__(self):
        self.plans=["Extreme weight loss","Weight loss","Mild weight loss","Maintain weight","Mild weight gain","Weight gain","Extreme weight gain"]
        self.weights=[0.6,0.8,0.9,1.0,1.1,1.2,1.4]
    ...
    def display_calories(self,person):
        ...
        for plan,weight,loss,col in zip(self.plans,self.weights,self.losses,st.columns(7)):
            with col:
                # Determine delta color based on whether it's weight loss or gain
                delta_color = "inverse" if "loss" in plan.lower() else "normal"
                st.metric(label=plan,value=f'{round(maintain_calories*weight)} Calories/day',delta=loss,delta_color=delta_color)
